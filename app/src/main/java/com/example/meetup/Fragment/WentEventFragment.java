...
import com.example.meetup.databinding.FragmentMypageWenteventBinding;
...
public class WentEventFragment extends Fragment {
    FragmentMypageWenteventBinding binding;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        binding= DataBindingUtil.inflate(inflater, R.layout.fragment_mypage_wentevent
                ,container,false);
        adapter = new EventAdapter(getContext());
        shared = new MyShared(getContext());
        TOKEN_GET = "bearer " + shared.get(TOKEN);
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(getContext());
        binding.rcvWent.setLayoutManager(linearLayoutManager);
            ...
            public void onResponse(Call<APIStatus> call, Response<APIStatus> response) {
                if (response.body().getStatus() == 1) {
                    ...
                    if (list.size() != 0) {
                        ...
                        binding.rcvWent.setAdapter(adapter);
                    } else {
